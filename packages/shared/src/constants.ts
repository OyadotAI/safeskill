...
export const DANGEROUS_MODULES = {
  ...
  network: ['http', 'https', 'net', 'dgram', 'node:http', 'node:https', 'node:net', 'node:dgram', 'node-fetch', 'axios', 'got', 'undici', 'request', 'superagent'],
...
export const PROMPT_INJECTION_PATTERNS = {
  instructionOverride: [
    ...
    /your\s+new\s+(role|instructions?|task|purpose)\s+(is|are)\s*:/i,
  ...
  dataExfiltration: [
    ...
    /send\s+(the\s+)?(contents?|files?|data|keys?|tokens?|credentials?|secrets?)\s+(to|via)/i,
    /include\s+(the\s+)?(contents?|files?|keys?|tokens?|credentials?)\s+in\s+(your\s+)?(response|output|reply|message)/i,
    /encode\s+(as|in|to)\s+base64/i,
    /exfiltrate/i,
    /upload\s+(the\s+)?(contents?|files?|data)\s+to/i,
  ...
  toolAbuse: [
    ...
    /add\s+(to|into)\s+(~\/)?\.(bashrc|zshrc|profile|ssh)/i,
  ...
  personaHijack: [
    ...
    /jailbreak/i,
    /bypass\s+(safety|content|ethical)\s+(filters?|restrictions?|guidelines?)/i,
